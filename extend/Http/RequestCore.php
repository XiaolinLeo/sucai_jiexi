...
class RequestCore
{
     ...
     */
    public $request_class = 'Http\RequestCore';

    /**
     * The default class to use for HTTP Responses (defaults to <ResponseCore>).
     */
    public $response_class = 'Http\ResponseCore';
...
//        curl_setopt($curl_handle, CURLOPT_CLOSEPOLICY, CURLCLOSEPOLICY_LEAST_RECENTLY_USED);
            ...
            case self::HTTP_PUT:
                ...
                if (isset($this->read_stream)) {
                    if (!isset($this->read_stream_size) || $this->read_stream_size < 0) {
                        throw new \think\Exception('The stream size for the streaming upload cannot be determined.', 10006);
                    }
    ...
    public function send_request($parse = false)
    {
        ...
        if ($this->response === false) {
            throw new \think\Exception('cURL resource: ' . (string) $curl_handle . '; cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')', 10006);
        }
