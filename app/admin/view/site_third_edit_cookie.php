<form class="card" method="post" autocomplete="off" action="{:url('admin/site/third_edit_cookie')}">
    <input type="hidden" name="third_id" value="{$third['third_id']}">
    {if !empty($cookie)}
        <input type="hidden" name="cookie_id" value="{$cookie['cookie_id']}">
    {/if}
    <div class="card-header">为 <a href="{:url('admin/site/third',['third_id'=>$third['third_id']])}">{$third['title']}</a> 新增Cookie - 第三方站点</div>
    <div class="card-body">
